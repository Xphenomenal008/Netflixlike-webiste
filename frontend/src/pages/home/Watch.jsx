...
import { ORG_IMG_URL, SMALL_IMG_URL } from "../../utils/constant";
import Watchskeleton from "../Watchskeleton";
import AiSummary from "./Ai_summery";
...
const Watch = () => {
  ...
  useEffect(() => {
    const getmovie = async () => {
      ...
      try {
        ...
        if (e.message.includes(404)) settrailer([]);
      } finally {
        setloading(false);
      }
    };
  ...
  useEffect(() => {
    const getmovie = async () => {
      ...
      try {
        ...
        if (e.message.includes(404)) setsimilercontent([]);
      } finally {
        setloading(false);
      }
    };
  ...
  const goToNextTrailer = () => {
    if (trailerIndex < trailer.length - 1) settrailerIndex(trailerIndex + 1);
  };

  const goToPreviousTrailer = () => {
    if (trailerIndex > 0) settrailerIndex(trailerIndex - 1);
  };

  // ✅ Similar movies scroll ref
          ...
          <div className="flex justify-center items-center gap-16 mx-auto mb-4 md:mb-0">
            <div className="flex-1">
              ...
              <p className="mt-2 text-lg ">
                {formrealesedata(
                  mymoviedetail?.release_date || mymoviedetail?.first_air_date
                )}{" "}
                |{" "}
                {mymoviedetail?.adult ? (
              ...
              <p className="mt-4 text-lg justify-stretch">
                {mymoviedetail.overview}
              </p>

              {/* ✅ AI Summarizer below overview */}
              <AiSummary overview={mymoviedetail.overview} />
            </div>

            <div className="flex-1">
              <img
                src={ORG_IMG_URL + mymoviedetail.poster_path}
                alt="poster_img"
          ...
          >
            ...
            {showarrow && (
              <>
                <button
                  onClick={leftslide}
                  className="absolute top-1/2 -translate-y-1/2 left-2 flex items-center justify-center size-12 rounded-full bg-black bg-opacity-50 hover:bg-opacity-75 text-white z-10"
                >
                  <ChevronLeft size={24} />
                </button>
                ...
                <button
                  onClick={rightslide}
                  className="absolute top-1/2 -translate-y-1/2 right-2 flex items-center justify-center size-12 rounded-full bg-black bg-opacity-50 hover:bg-opacity-75 text-white z-10"
                >
                  <ChevronRight size={24} />
                </button>
              </>
