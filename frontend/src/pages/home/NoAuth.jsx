  ...
  return (
    <>
      <div className='relative h-screen text-white overflow-hidden'>
        ...
        <div className='absolute top-0 left-0 w-full h-full'>
          {trending?.backdrop_path ? (
            <>
              {imgload && (
                <div className='absolute top-0 left-0 w-full h-full bg-gradient-to-br from-slate-900 to-black -z-50' />
              )}
              <img 
            ...
            <div className='w-full h-full bg-gradient-to-br from-slate-900 via-black to-black -z-50' />
          )}
        ...
        <div className='relative z-10 h-full flex flex-col justify-center px-4 sm:px-8 md:px-16 lg:px-32 py-20 max-w-3xl'>
          <h1 className='flex flex-col gap-2'>
            <span className='text-5xl sm:text-6xl lg:text-7xl font-extrabold text-balance leading-tight'>
              {trending?.title || trending?.name || 'Featured Content'}
            </span>
            <span className='text-base sm:text-lg text-gray-300'>
              {trending?.release_date?.split('-')[0] || trending?.first_air_date?.split('-')[0] || 'N/A'} | {trending?.adult ? '18+' : 'PG-13'}
            </span>
          </h1>
         
          <p className='text-sm sm:text-base lg:text-lg text-gray-200 leading-relaxed mt-6 max-w-2xl'>
            {trending?.overview 
              ? (trending.overview.length > 220 ? trending.overview.slice(0, 220) + '...' : trending.overview) 
              : 'Experience premium entertainment with our curated selection.'}
          </p>
          <div className='flex gap-3 mt-6'>
            <Link to={`/watch/${trending?.id}`}>
              <button className='flex gap-2 bg-white hover:bg-gray-200 text-black px-5 sm:px-8 py-2 sm:py-3 font-bold rounded transition items-center text-sm sm:text-base'>
                <Play size={20} className='fill-black' /> Play
              </button>
            </Link>
            <Link to='/'> 
              <button className='flex gap-2 bg-gray-600 hover:bg-gray-700 text-white px-5 sm:px-8 py-2 sm:py-3 font-bold rounded transition items-center text-sm sm:text-base'>
      ...
      <div className='flex flex-col gap-10 bg-black py-10 text-white'>
        {contentType === 'movies' 
          ? MOVIE_CATEGORY.map((item, ind) => <Showlist key={ind} item={item} />)
          : TV_CATEGORY.map((item, ind) => <Showlist key={ind} item={item} />)
        }
      </div>
    </>
  )
